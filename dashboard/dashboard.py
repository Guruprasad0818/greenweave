"""
GreenWeave — Dashboard v2.1
Climate AI Control Center: Carbon Budget + Predictive Carbon + Follow-the-Sun
Multi-Region + Self-Learning Engine + Semantic Cache panels.
...
/* Per-message routing badges */
.route-badge{display:inline-flex;align-items:center;gap:6px;font-family:var(--mono);font-size:11px;font-weight:700;padding:4px 12px;border-radius:100px;letter-spacing:0.5px;margin-bottom:8px;}
.route-badge.live{background:#00ff8820;color:#00ff88;border:1px solid #00ff8840;}
.route-badge.cache{background:#b388ff20;color:#b388ff;border:1px solid #b388ff40;}
...
with st.sidebar:
    ...
    if budget_set:
        ...
        if st.button("↺ Reset Carbon Budget", use_container_width=True):
            api("/budget/reset","post")
            st.success("Carbon budget usage reset to 0.")
            st.rerun()
    ...
    if not router_ok:
        st.warning("⚠️ Cannot reach the Elastic Router — showing last-known / fallback values.")
...
for msg in st.session_state.messages:
    if msg["role"] == "user":
        ...
        badge = ('<span class="route-badge cache">🟣 Zero-Carbon Cache Hit</span>' if is_cache
                 else '<span class="route-badge live">🟢 Live Routing</span>')
        st.markdown(f'<div class="bubble-label">GREENWEAVE AI</div>{badge}<div class="bubble-ai">{msg["content"]}</div>', unsafe_allow_html=True)
...
if send and user_input.strip():
    ...
    with st.spinner("🌿 Checking semantic cache & carbon routing…"):
        try:
            ...
            st.warning("⚠️ Cannot reach the Elastic Router at elastic_router:8000. Is the backend running?")
        except requests.exceptions.Timeout:
            st.warning("⚠️ The router took too long to respond. Please try again.")
        except requests.exceptions.HTTPError as e:
            st.error(f"⚠️ Router returned an error: {e}")
