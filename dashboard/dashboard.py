    ...
    if msg["role"] == "user":
        ...
        if receipt:
            ...
            if b_limit:
                ...
                budget_row = f"""
                <div class="receipt-row" style="margin-top:8px;border-top:1px solid #1e3024;padding-top:8px;">
                    <span>Budget Used</span><span>{b_used:.4f}g / {b_limit}g</span>
                </div>
                <div class="receipt-row {b_color_class}">
                    <span>Budget %</span><span>{b_pct:.1f}% — {pressure_label(b_pressure)}</span>
                </div>
                """
...
if send and user_input.strip():
    ...
    with st.spinner("🌿 Routing to climate-appropriate model…"):
        try:
            ...
            st.error("⚠️ Cannot reach the Elastic Router at localhost:8000")
